...
fn compute_monthly_interest_rate(
    annual_interest_rate: f64,
    interest_style: &CompoundingInterestStyle,
) -> f64 {
...
fn compute_monthly_payments(
    ...
    let mut payment_guess = (principal
        * compute_monthly_interest_rate(annual_interest, compounding_style))
        + max_step_size;
    ...
    loop {
        let term_length_guess =
            how_long_to_pay(payment_guess, annual_interest, principal, compounding_style);
        match phase {
            CurrentPhase::Broad => {
...
pub fn run_house_simulator(params: HouseSimulatorParameters) -> HousingSimulatorResults {
    let monthly_payment = compute_monthly_payments(
        ...
        params.term_length_years,
        &params.compounding_style,
    );
...
mod tests {
    ...
    fn compute_term_length_test() {
        let how_many_months =
            how_long_to_pay(1787.75, 5.25, 300000.0, &CompoundingInterestStyle::Canadian);
    ...
    fn compute_monthly_payments_test() {
        let monthly =
            compute_monthly_payments(300000.0, 5.25, 25, &CompoundingInterestStyle::Canadian);
